...
Status: **shipped (EB-1–EB-4).** Verified by `test/event-bus-host.test.ts` and `test/parallel-stream-chrome.test.ts`. PU-3 and PU-4 now own bounded and dependency-aware scheduling. Implementation-allocation notes below are historical.
P1 shared state is shipped: `HostEventBus` assigns ordered run-scoped envelopes for `SPEC_PUBLISHED`, `ARCHITECTURE_READY`, `TASK_READY`, `TASK_COMPLETED`, and `TASK_FAILED`; duplicate/stale envelopes are rejected, and no event contains bot chat histories. Verified by `test/shared-state-events.test.ts` and `test/work-run-host.test.ts`.
...
Date: 2026-09-02.
Parent: `architecture-mvp.md`. Isolation: `architecture-bot-isolation.md` (SI-1/2/4 held; SI-3 reopened here). OpenSpec: `architecture-openspec-trace.md` (OS-4 bodies stay required talk). Pack: `architecture-token-save.md` (QC-3 unchanged; overflow **that bot only** in a batch). Context Map: `architecture-context-map.md` (CM-4 extras only). Copilot stays `vscode.lm`. ₹0 extra keys. No second runtime. F8a Work reuses this in-process bus; Debate composer-lock is Debate-only; Work is a different run type — [architecture-work-run.md](./architecture-work-run.md).
...
**PU-5 current-runtime note (2026-09-08):** the scheduling and settle-then-ingest guarantees below remain authoritative, while PU-5 supersedes the old phase names and convergence rule. Current Debate is one proposal batch → one settled synthesis → one targeted objection batch → sequential host-evaluated votes. Historical “critique” references below map to the PU-5 objection batch; quorum/majority, configured blockers, high-risk unanimity, and the automatic cap live in `debate-policy.ts`.