...
Stories: **SI-1–4 is the full story set.** **SI-1** per-bot `LanguageModelChatMessage[]` session store (session-only; no global transcript stuffed into every pack). **SI-2** controlled ingest: structured packets (`requirements` / `decisions` / `constraints` / `openQuestions`) verbatim; drop banter + failed drafts only; never lossy-summarize away acceptance criteria. **SI-3** publish at end of each meaningful bot turn + consensus / Pick. **SI-4** downstream: bots with a remaining turn in this run (including prior speakers if they speak again) + implementer; not inactive bots.
...
- Each bot has its own in-memory session: `LanguageModelChatMessage[]` (system / persona + that bot’s own turns + ingested packets). Session-only: reload / end of run clears with the Swarm transcript.
- Host does **not** restuff the full global Swarm transcript into every Copilot pack (QC already stopped that; this slice formalizes per-bot history + handoffs).
- Controlled publish of structured packets only. Fields: `requirements`, `decisions`, `constraints`, `openQuestions` (host-owned strings / lists). **Verbatim.** Drop debate banter and failed drafts only. Never lossy-summarize away acceptance criteria.
- Publish at end of each meaningful bot turn (propose / critique / direct that produced lasting content) and on consensus / Pick.
- Downstream receivers: bots that still have a remaining turn in this sequential run (including ones that already spoke if they will speak again) + implementer. Not inactive bots. Not bots that are done for this run.
- Sequential orchestrator unchanged: one `sendRequest` at a time. No Event Bus. No concurrent `vscode.lm`. No parallel F7.
- TokenGovernor: attachment / MCP-style extras still trim silent first. **Required published packets for this turn are NOT silent extras.** If they cannot fit with prompt + board + (LSP slice OR implementer files) + tab paths → existing QC-3 `error` `code: 'pack-overflow'`, no `sendRequest`, composer enabled.
- Visible Swarm stays full HV prose. No new Swarm chrome for isolation. No tree / form chrome. No fourth sidebar.
- BR / QC / HV / MA / SD / TA / MS frozen otherwise. Leftovers 002 / 003 / 009 / 014 out. Graphify out.
...
Do **not** enqueue for inactive bots (unless this-turn `@` solo — that bot is the downstream).
...
3. If still over → pack-overflow (QC-3)

No restuff of full ThreadStore transcript into packs.
...
- Each bot’s Copilot pack does not include other bots’ full HV articles as transcript restuff.
- Published packet fields appear verbatim in downstream bot packs.
- Banter / failed drafts are not published as requirements.
- Inactive bots do not receive inbox packets.
- Implementer receives packets before implement pack.
- Required packets that cannot fit → pack-overflow; no `sendRequest`.
- Attachment extras still trim silent.
- Sequential: no overlapping `sendRequest`.
- Reload clears sessions.
- No new UI protocol members.
