...
Status: **shipped (SI-1–SI-4, including the SI-3 parallel-batch revision).** Verified by `test/isolation-host.test.ts` and `test/event-bus-host.test.ts`. Implementation-allocation notes below are historical.
