...
Status: **shipped (CM-1–CM-4).** Verified by `test/context-map-host.test.ts` and `test/context-map-chrome.test.ts`. CTX-1 and CTX-3 now own whole-repository indexing and requirement links. Implementation-allocation notes below are historical.
...
Addendum 2026-09-08: CTX-1–3 repository context is additive — openspec/specs/ctx-1-repository-context/spec.md. `RepositoryContextService` builds a **separate** local file/symbol/import/call/Markdown index for prompt assembly. The Context Map view (CM-1–4) stays on-demand and does **not** crawl the whole workspace. CTX indexing and CM neighborhood graphs are independent surfaces.
