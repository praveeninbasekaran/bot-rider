...
Status: **shipped (EX-1–EX-4).** Verified by `test/bot-export.test.ts` and `test/bot-form-export-chrome.test.ts`. Implementation-allocation notes below are historical.
