...
Architecture: [architecture-bot-model.md](./architecture-bot-model.md). Additive. **MS-1–3 locked.** Copilot vendor only via `vscode.lm`. Persist on the bot record.

## 22. Per-bot model picker (F5)

**Status:** Additive. **MS-1–3 locked.** Copilot vendor only via `vscode.lm`. Persist on the bot record. Not a Swarm control. Not a fourth sidebar. Not token chrome. Not F7 parallel. Not leftovers 002/003/009/014.
...
Host discovers with `vscode.lm.selectChatModels({ vendor: 'copilot' })` only (form open is enough user gesture). Refresh on `vscode.lm.onDidChangeChatModels` while the form is open. **No other vendors.**
...
- UI → Host on Save: create/update plus `modelId: string | null` (id key only)
